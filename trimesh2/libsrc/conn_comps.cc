...
static void find_connected(const TriMesh *mesh,
			   ...
			   size_t f, size_t whichcomponent, bool conn_vert)
{
	stack<size_t> s;
	s.push(f);
	while (!s.empty()) {
...
void select_big_comps(TriMesh *mesh,
		      const vector<int> &comps, const vector<int> &compsizes,size_t min_size,
		      size_t total_largest /* = std::numeric_limits<int>::max() */)
{
	size_t ncomp = compsizes.size();
	size_t keep_last = min(ncomp - 1, total_largest - 1);
...
void select_small_comps(TriMesh *mesh,
	...
	size_t max_size,
	size_t total_smallest /* = std::numeric_limits<int>::max() */)
{
	size_t ncomp = compsizes.size();
	size_t keep_first = max((size_t) 0, ncomp - total_smallest);
